...
const mainFile = findJs("main bundle with remote-control remote-control flow", (_file, text) =>
  (text.includes("desktop_fetch_auth_401") ||
    ...
    text.includes("async function R_({action:e,appServerClient:t")) &&
  ...
  (text.includes("__codexRemoteControlAuthOverrideForPath") ||
    ...
    text.includes("async function w2")) &&
  (text.includes("remote_control_desktop_fetch_override_used") ||
    ...
    (text.includes("async function XF({appServerClient:e,errorStatus:t,failureMessage:n,refreshToken:r,state:i}") &&
      text.includes("async function z_({action:e,appServerClient:t")) ||
    (text.includes("async function ZF({appServerClient:e,errorStatus:t,failureMessage:n,refreshToken:r,state:i}") &&
      text.includes("async function R_({action:e,appServerClient:t")))
);
...
function patchFlowHelpers(text) {
  ...
  const anchor = [
    ...
    "var m2=`app_EMoamEEZ73f0CkXaXp7hrann`",
...
function patchDesktopFetch(text) {
  ...
  const hasPathAwareDesktopFetch =
    ...
    text.includes("async function ZF({appServerClient:e");
  ...
  if (
    ...
    const oldInitialAuth =
      "d=await ZF({appServerClient:this.getAppServerConnection(V),errorStatus:432,failureMessage:`Failed to retrieve authentication token`,refreshToken:!1,state:d})";
    const newInitialAuth =
      "d=await ZF({appServerClient:this.getAppServerConnection(V),errorStatus:432,failureMessage:`Failed to retrieve authentication token`,refreshToken:!1,state:d,resolvedUrl:a})";
    const oldRetryAuth =
      "d=await ZF({appServerClient:this.getAppServerConnection(V),errorStatus:401,failureMessage:`Failed to refresh authentication token`,refreshToken:!0,state:d})";
    const newRetryAuth =
      "d=await ZF({appServerClient:this.getAppServerConnection(V),errorStatus:401,failureMessage:`Failed to refresh authentication token`,refreshToken:!0,state:d,resolvedUrl:a})";
    let next = replaceExact(text, oldZf12708, newZf12708, "desktop_fetch 26.707.12708 auth fallback");
    next = replaceExact(next, oldInitialAuth, newInitialAuth, "desktop_fetch 26.707.12708 initial auth call");
    next = replaceExact(next, oldRetryAuth, newRetryAuth, "desktop_fetch 26.707.12708 retry auth call");
    if (!next.includes(newPathMarker)) {
      throw new Error("desktop_fetch 26.707.12708 auth path marker missing after patch");
    }
    return { text: next, status: "patched-26.707.12708-auth-path" };
  }
  if (
    text.includes("async function XF({appServerClient:e,errorStatus:t,failureMessage:n,refreshToken:r,state:i}") &&
    text.includes("async function z_({action:e,appServerClient:t")
...
function patchAppServerAuthFallback(text) {
  ...
  if (!next.includes(marker)) {
    ...
    const oldR12708 =
      "async function R_({action:e,appServerClient:t,desktopOriginator:n,headers:r={},refreshToken:i=!1}){let o=await t.getAuthToken({refreshToken:i});if(!o){let t=a.W();throw Error(t===`ChatGPT`?`Sign in to ChatGPT to ${e}.`:`Sign in to ChatGPT in ${t} to ${e}.`)}let s={...r};return z_(s,o,{desktopOriginator:n}),s}";
    const newR12708 =
      "async function R_({action:e,appServerClient:t,desktopOriginator:n,headers:r={},refreshToken:i=!1}){let o=await t.getAuthToken({refreshToken:i});if(!o)try{o=typeof __codexRemoteControlConnectionAuthOverride==\"function\"?__codexRemoteControlConnectionAuthOverride():null,o&&typeof __codexRemoteControlFlowLog==\"function\"&&(__codexRemoteControlFlowLog(\"remote_control_appserver_bh_isolated_auth_fallback\",{action:e,refreshToken:i}),__codexRemoteControlFlowLog(\"remote_control_desktop_fetch_override_used\",{path:\"/codex/remote/control\",action:e}))}catch(t){try{typeof __codexRemoteControlFlowLog==\"function\"&&__codexRemoteControlFlowLog(\"remote_control_appserver_bh_isolated_auth_fallback_failed\",{action:e,errorName:t?.name,errorMessage:t?.message,errorCode:t?.code})}catch{}}if(!o){let t=a.W();throw Error(t===`ChatGPT`?`Sign in to ChatGPT to ${e}.`:`Sign in to ChatGPT in ${t} to ${e}.`)}let s={...r};return z_(s,o,{desktopOriginator:n}),s}";
          ...
          : next.includes(oldV_)
            ...
            : next.includes(oldN_2623)
              ...
              : next.includes(oldZ_)
                ...
                : replaceExact(next, oldR12708, newR12708, "remote-control app-server auth fallback R_");
    status = "patched";
  }
...
function patchStepUpFlow(text) {
  ...
  const oldW212708 =
    "async function w2({accountId:e,desktopApiOptions:t,fetchToken:n=(e,t)=>c.net.fetch(e,t),openExternalUrl:r=e=>LA({request:{url:e,initiator:`open_in_browser_bridge`,openTarget:`external-browser`}}),timeoutMs:i=y2}){let a=E2(),o=O2(),s=k2(32),l=await j2({state:s,timeoutMs:i});l.authorizationCode.catch(()=>{});try{if(await r(T2({issuer:a,clientId:m2,redirectUri:l.redirectUri,codeChallenge:o.codeChallenge,state:s,originator:t.desktopOriginator,accountId:e}))===!1)throw Error(`Failed to open remote control login in the default browser.`);return(await A2({code:await l.authorizationCode,codeVerifier:o.codeVerifier,clientId:m2,issuer:a,redirectUri:l.redirectUri,fetchToken:n})).access_token}finally{l.close()}}";
  const newW212708 =
    "async function w2({accountId:e,desktopApiOptions:t,fetchToken:n=(e,t)=>c.net.fetch(e,t),openExternalUrl:r=e=>LA({request:{url:e,initiator:`open_in_browser_bridge`,openTarget:`external-browser`}}),timeoutMs:i=y2}){let __codexRemoteControlCachedStepUp=typeof __codexRemoteControlReadFreshStepUpToken==\"function\"?__codexRemoteControlReadFreshStepUpToken(e):null;if(__codexRemoteControlCachedStepUp)return __codexRemoteControlCachedStepUp;let a=E2(),o=O2(),s=k2(32),l=await j2({state:s,timeoutMs:i});l.authorizationCode.catch(()=>{});try{__codexRemoteControlFlowLog(\"remote_control_step_up_browser_open\",{issuer:a,redirectUri:l.redirectUri,accountId:e??null});if(await r(T2({issuer:a,clientId:m2,redirectUri:l.redirectUri,codeChallenge:o.codeChallenge,state:s,originator:t.desktopOriginator,accountId:e}))===!1)throw Error(`Failed to open remote control login in the default browser.`);__codexRemoteControlFlowLog(\"remote_control_step_up_wait_callback\",{redirectUri:l.redirectUri});let __codexRemoteControlCode=await l.authorizationCode;__codexRemoteControlFlowLog(\"remote_control_step_up_callback_received\",{codeLength:typeof __codexRemoteControlCode==\"string\"?__codexRemoteControlCode.length:null});return(await A2({code:__codexRemoteControlCode,codeVerifier:o.codeVerifier,clientId:m2,issuer:a,redirectUri:l.redirectUri,fetchToken:n})).access_token}catch(e){__codexRemoteControlFlowLog(\"remote_control_step_up_failed\",{errorName:e?.name,errorMessage:e?.message,errorCode:e?.code,stack:e?.stack});throw e}finally{l.close();__codexRemoteControlFlowLog(\"remote_control_step_up_listener_closed\",{})}}";
  const oldA212708 =
    "async function A2({code:e,codeVerifier:t,clientId:n,issuer:r,redirectUri:i,fetchToken:a}){let o=await a(new URL(`/oauth/token`,D2(r)).toString(),{method:`POST`,headers:{\"Content-Type\":`application/x-www-form-urlencoded`},body:new URLSearchParams({grant_type:`authorization_code`,code:e,redirect_uri:i,client_id:n,code_verifier:t}).toString()});if(!o.ok)throw Error(`Remote control step-up token exchange failed with status ${o.status}.`);return C2.parse(await o.json())}";
  const newA212708 =
    "async function A2({code:e,codeVerifier:t,clientId:n,issuer:r,redirectUri:i,fetchToken:a}){__codexRemoteControlFlowLog(\"remote_control_step_up_token_exchange_started\",{issuer:r,redirectUri:i});let o=await a(new URL(`/oauth/token`,D2(r)).toString(),{method:`POST`,headers:{\"Content-Type\":`application/x-www-form-urlencoded`},body:new URLSearchParams({grant_type:`authorization_code`,code:e,redirect_uri:i,client_id:n,code_verifier:t}).toString()});if(!o.ok){let e=\"\";try{e=await o.text()}catch{}__codexRemoteControlFlowLog(\"remote_control_step_up_token_exchange_failed\",{status:o.status,bodySnippet:e.slice(0,500)});throw Error(\"Remote control step-up token exchange failed with status \"+o.status+\".\")}let s=await o.json(),c=C2.parse(s);try{__codexRemoteControlStoreStepUpTokenResponse(c,{source:String(i).includes(\"/deviceauth/callback\")?\"device_code\":\"pkce\",issuer:r,redirectUri:i})}catch(e){__codexRemoteControlFlowLog(\"remote_control_step_up_store_failed\",{errorName:e?.name,errorMessage:e?.message,errorCode:e?.code})}__codexRemoteControlFlowLog(\"remote_control_step_up_token_exchange_done\",{status:o.status,responseKeys:c&&typeof c==\"object\"?Object.keys(c).slice(0,20):[]});return c}";
          ...
          : next.includes(oldC$)
            ...
            : next.includes(oldI1)
              ...
              : next.includes(oldS2)
                ...
                : replaceExact(next, oldW212708, newW212708, "remote-control step-up w2");
    status = "patched";
  }
          ...
          : next.includes(oldM$)
            ...
            : next.includes(oldH1)
              ...
              : next.includes(oldO2)
                ...
                : replaceExact(next, oldA212708, newA212708, "remote-control step-up token exchange");
    status = "patched";
  }
  return { text: next, status };
}
...
function patchRemoteControlHttp(text) {
  ...
  const oldEv12708 =
    "async function ev({action:e,appServerClient:t,desktopApiOptions:n,path:r,method:i,headers:a={},body:o,mapNotFoundToFeatureUnavailable:s=!0}){let l=L_(n,r),u=await tv({action:e,appServerClient:t,desktopApiOptions:n,headers:a}),d=await c.net.fetch(l,{method:i,headers:u,body:o});if(d.status===401&&(u=await tv({action:e,appServerClient:t,desktopApiOptions:n,headers:a,refreshToken:!0}),d=await c.net.fetch(l,{method:i,headers:u,body:o})),d.status===404&&s)throw new J_;if(d.status===403)throw new X_(await cv(d));if(d.status===401)throw new Y_(nv(e));if(!d.ok)throw Error(`Remote control request failed (${d.status}): ${await cv(d)}`);return d}";
  const newEv12708 =
    "async function ev({action:e,appServerClient:t,desktopApiOptions:n,path:r,method:i,headers:a={},body:o,mapNotFoundToFeatureUnavailable:s=!0}){let l=L_(n,r),u=await tv({action:e,appServerClient:t,desktopApiOptions:n,headers:a}),d=await c.net.fetch(l,{method:i,headers:u,body:o});__codexRemoteControlFlowLog(\"remote_control_http_response\",{path:r,method:i,status:d.status,ok:d.ok,refreshed:!1});if(d.status===401){u=await tv({action:e,appServerClient:t,desktopApiOptions:n,headers:a,refreshToken:!0});d=await c.net.fetch(l,{method:i,headers:u,body:o});__codexRemoteControlFlowLog(\"remote_control_http_response\",{path:r,method:i,status:d.status,ok:d.ok,refreshed:!0})}if(!d.ok){let e=\"\";try{let t=d.clone?d.clone():d;e=await t.text()}catch(e){e=\"<<body read failed: \"+(e?.message??e)+\">>\"}__codexRemoteControlFlowLog(\"remote_control_http_failure_body\",{path:r,status:d.status,bodySnippet:e.slice(0,500)})}if(d.status===404&&s)throw new J_;if(d.status===403)throw new X_(await cv(d));if(d.status===401)throw new Y_(nv(e));if(!d.ok)throw Error(\"Remote control request failed (\"+d.status+\"): \"+await cv(d));return d}";
  return {
          ...
          : text.includes(oldP_)
            ...
            : text.includes(oldY_)
              ...
              : text.includes(oldTv)
                ...
                : replaceExact(text, oldEv12708, newEv12708, "remote-control HTTP diagnostics"),
    status: "patched",
  };
}
...
function patchRemoteControlAuthorize(text) {
  ...
  const oldVv12708 =
    "async function vv({appServerClient:e,desktopApiOptions:t,deviceKeyClient:n,globalState:r,requestRemoteControlEnrollmentStepUpToken:i}){await Cv({appServerClient:e,deviceKeyClient:n,desktopApiOptions:t,enrollmentKey:bv(t),globalState:r,headers:await Sv({action:`authorize remote control environments`,appServerClient:e,desktopApiOptions:t}),requestRemoteControlEnrollmentStepUpToken:i})}";
  const newVv12708 =
    "async function vv({appServerClient:e,desktopApiOptions:t,deviceKeyClient:n,globalState:r,requestRemoteControlEnrollmentStepUpToken:i}){__codexRemoteControlFlowLog(\"remote_control_qm_start\",{hasStepUp:typeof i==\"function\"});try{let a=await Sv({action:`authorize remote control environments`,appServerClient:e,desktopApiOptions:t});__codexRemoteControlFlowLog(\"remote_control_qm_headers_ready\",{headerKeys:Object.keys(a).filter(e=>e.toLowerCase()!==\"authorization\").sort(),hasAuthorization:Object.keys(a).some(e=>e.toLowerCase()===\"authorization\"),hasChatGptAccountId:Object.keys(a).some(e=>e.toLowerCase()===\"chatgpt-account-id\")});await Cv({appServerClient:e,deviceKeyClient:n,desktopApiOptions:t,enrollmentKey:bv(t),globalState:r,headers:a,requestRemoteControlEnrollmentStepUpToken:i});__codexRemoteControlFlowLog(\"remote_control_qm_completed\",{})}catch(e){__codexRemoteControlFlowLog(\"remote_control_qm_failed\",{errorName:e?.name,errorMessage:e?.message,errorCode:e?.code,stack:e?.stack});throw e}}";
  return {
          ...
          : text.includes(oldQ_)
            ...
            : text.includes(oldPv)
              ...
              : text.includes(oldYv)
                ...
                : replaceExact(text, oldVv12708, newVv12708, "remote-control authorize flow"),
    status: "patched",
  };
}
...
function patchDeviceKeyCreationLogs(text) {
  ...
  const oldCreate12708 =
    "async function Zv({accountUserId:e,clientId:t,deviceKeyClient:n}){let r=await n.createDeviceKey(`allow_os_protected_nonextractable`);return{accountUserId:e,algorithm:r.algorithm,clientId:t,keyId:r.keyId,protectionClass:r.protectionClass,publicKeySpkiDerBase64:r.publicKeySpkiDerBase64}}";
  const newCreate12708 =
    "async function Zv({accountUserId:e,clientId:t,deviceKeyClient:n}){__codexRemoteControlFlowLog(\"remote_control_create_device_key_start\",{});try{let r=await n.createDeviceKey(`allow_os_protected_nonextractable`);return __codexRemoteControlFlowLog(\"remote_control_create_device_key_done\",{algorithm:r.algorithm,protectionClass:r.protectionClass}),{accountUserId:e,algorithm:r.algorithm,clientId:t,keyId:r.keyId,protectionClass:r.protectionClass,publicKeySpkiDerBase64:r.publicKeySpkiDerBase64}}catch(e){__codexRemoteControlFlowLog(\"remote_control_create_device_key_failed\",{errorName:e?.name,errorMessage:e?.message,errorCode:e?.code});throw e}}";
  return {
          ...
          : text.includes(oldCreate2616)
            ...
            : text.includes(oldCreate2623)
              ...
              : text.includes(oldCreate26707)
                ...
                : replaceExact(text, oldCreate12708, newCreate12708, "remote-control device key creation logs"),
    status: "patched",
  };
}
...
function patchSoftwareDeviceKeyFallback(text) {
  ...
  const new$2 = newPq
    .replace("function pQ({resourcesPath:e})", "function $2({resourcesPath:e})")
    .replace("lQ((0,s.join)(e,`native`,uQ))", "Y2((0,u.join)(e,`native`,X2))")
    .replaceAll("mQ(t)", "e4(t)");
  return {
          ...
          : text.includes(oldL$)
            ...
            : text.includes(oldD0)
              ...
              : text.includes(oldZ2)
                ...
                : replaceExact(text, old$2, new$2, "remote-control software device-key fallback"),
    status: "patched",
  };
}
...
function patchMobileSetupMfaInfo403(text) {
  ...
  const oldMfaInfo12708 =
    "async function Ee(){return E.parse(await g.safeGet(`/accounts/mfa_info`)).mfa_enabled_v2}";
  const newMfaInfo12708 =
    "async function Ee(){try{return E.parse(await g.safeGet(`/accounts/mfa_info`)).mfa_enabled_v2}catch(e){if(e instanceof re&&e.status===403)return void\"remote_control_mfa_info_403_nonblocking\",!0;throw e}}";
  let next;
  ...
  } else if (text.includes(oldMfaInfo12708)) {
    next = replaceExact(text, oldMfaInfo12708, newMfaInfo12708, "mobile setup MFA info 403 fallback 26.707.12708");
  } else {
