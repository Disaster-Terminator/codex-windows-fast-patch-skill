...
function Get-OrCreateSigningCertificate {
  ...
  try {
    ...
    $trustedCandidate = $myStore.Certificates |
      Where-Object {
        ...
        (Test-CertificateInStore -Thumbprint $_.Thumbprint -StoreName ([System.Security.Cryptography.X509Certificates.StoreName]::Root) -StoreLocation ([System.Security.Cryptography.X509Certificates.StoreLocation]::LocalMachine))
      } |
  ...
  $basePath = Join-Path $script:OperationTempRoot ('codex-remote-signing-' + [guid]::NewGuid().ToString('N'))
...
try {
  ...
  Invoke-SignPackage -SignTool $signtool -MsixPath $msixPath -Cert $cert
