...
if (!next.includes(workAccessMarker)) {
  if (next.includes('CODEX_POWER_SLIDER_V1')) {
    next = next.replace('/*CODEX_POWER_SLIDER_V1*/', '/*CODEX_POWER_SLIDER_V1*//*CODEX_WORK_MODE_ACCESS_V1*/');
  } else {
...
if (text.includes('CODEX_WORK_MODE_ACCESS_V1')) {
  const next = text.replace('/*CODEX_WORK_MODE_ACCESS_V1*/', '/*CODEX_WORK_MODE_ACCESS_V1*//*CODEX_POWER_SLIDER_V1*/');
  fs.writeFileSync(file, next);
  process.stdout.write('patched');
  process.exit(0);
}
...
function Invoke-PatchAppAsar {
  ...
  if ($OnlyModelExperience) {
    ...
    foreach ($syntaxTarget in @($fastModeTarget, $fastModeUiTarget, $customModelsTarget, $powerSliderTarget, $modelPickerTarget) | Sort-Object -Unique) {
  ...
  foreach ($syntaxTarget in @($targets.FastMode, $targets.FastModeUi, $targets.CustomModels, $targets.PowerSlider, $targets.ModelPicker) | Sort-Object -Unique) {
    & $nodePath --check $syntaxTarget
    if ($LASTEXITCODE -ne 0) {
      Fail "Model Experience patched asset failed node --check: $syntaxTarget"
    }
