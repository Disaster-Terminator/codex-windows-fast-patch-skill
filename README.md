...
- `scripts/build-remote-control-native-replacement.ps1`：当 native app-server 因 API-key 主认证拒绝手机远控时，在指定工作目录下构建 patched `app\resources\codex.exe` replacement。默认从安装包副本自动识别原生版本；内置映射包括在 Desktop `26.707.3748.0` 上完成构建/安装/手机消息往返验证的 `0.144.0-alpha.4`、在 Desktop `26.707.12708.0` 上完成 MSVC 构建、MSIX dry-run/签名安装、live marker 和隔离认证接口验证的 `0.144.5`，以及仅通过 patch-apply 验证的历史 `0.142.4`。`0.144.5` 尚未完成本机 enrollment 和手机消息往返，不能表述为完整端到端验证。其他版本必须提供严格匹配的 `-CodexSourceRef`、`-AppServerVersion` 和已验证的 `-PatchPathOverride`。
...
- Fast Mode 本地线缆验证能在 Codex Desktop 的 `/v1/responses` HTTP 请求体或 WebSocket 帧里捕获 `service_tier=priority`。
