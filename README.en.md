...
- `scripts/build-remote-control-native-replacement.ps1`: Builds the patched native `app\resources\codex.exe` replacement under a caller-selected work root when the native app-server rejects API-key main auth. By default it detects the installed native version from a copied executable; bundled mappings cover `0.144.0-alpha.4`, validated through build, install, and a phone-message round trip with Desktop `26.707.3748.0`; `0.144.5`, validated with Desktop `26.707.12708.0` through MSVC build, MSIX dry-run/signed install, live markers, and isolated-auth endpoint verification but not yet enrollment or a phone-message round trip; plus historical patch-apply-only validated `0.142.4`. Any other version requires an exact `-CodexSourceRef` / `-AppServerVersion` pair plus a validated `-PatchPathOverride`.
...
- Fast Mode local wire verification captures `service_tier=priority` from Codex Desktop's `/v1/responses` HTTP body or WebSocket frame.
