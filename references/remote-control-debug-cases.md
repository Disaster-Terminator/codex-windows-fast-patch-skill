...
Use the printed `ReplacementResourceCodexExe` value with `patch-remote-control-windows-msix.ps1 -ReplacementResourceCodexExe`. With both version parameters omitted, the helper copies the installed WindowsApps native binary into `WorkRoot\tmp`, runs `--version` on the copy, and chooses a known mapping without executing the package binary in place. Codex Desktop `26.707.3748.0` ships native `codex-cli 0.144.0-alpha.4`, and Desktop `26.707.12708.0` ships `codex-cli 0.144.5`; both map to `references\remote-control-native-replacement.patch`. Use a short work root such as `G:\rc` because Cargo git checkouts can otherwise exceed Windows path limits. For historical `rust-v0.142.4`, matching parameters select `references\remote-control-native-replacement-0.142.4.patch`; this older patch has passed clean patch-apply validation only. Any other version requires exact matching version parameters and an explicitly validated `-PatchPathOverride`. The helper checks patch applicability before compilation and keeps all build/cache/temp output under `-WorkRoot`.
...
Point-in-time validation: Codex Desktop `26.707.12708.0` with native `0.144.5` completed exact source/tag verification, MSVC build, remote-control ASAR/native dry-run, signed Developer MSIX install, live marker checks, isolated `remote.json` refresh, and `/backend-api/wham/remote/control/clients` verification on 2026-07-16. The main app remained API-key authenticated with `model_provider = "codex_local_access"`. Enrollment and a phone-message round trip were not yet completed, so this is not full end-to-end validation.

When MSVC is installed but `kernel32.lib` is unavailable, the helper accepts only a coherent SDK root/version containing matching x64 libraries and headers. If none is usable, it downloads `Microsoft.Windows.SDK.CPP` plus `Microsoft.Windows.SDK.CPP.x64` `10.0.26100.4188` under `<WorkRoot>\cache\windows-sdk-cpp`. NuGet downloads go to `.partial`, are opened as ZIPs and checked for expected payloads before replacing the formal cache, and transport or validation failures are cleaned and retried up to three times. It honors `HTTPS_PROXY` / `HTTP_PROXY`, otherwise uses `http://127.0.0.1:10808` only if listening, and falls back to direct download. Point `-WorkRoot` at `D:\...` or another requested large drive to keep source, compilation, NuGet payloads, and temp files off `C:`. `-SkipBuild` requires the matching build-stamp JSON and skips SDK initialization entirely.
...
9. After successful remote-control install, verify that the ordinary patched features survived. At minimum run `install-computer-use-local.ps1 -StrictVerifyOnly`, `codex plugin list`, and a Windows sandbox smoke test. If strict verification reports a stale Chrome native-host manifest or a missing/stale bundled cache link, run `install-computer-use-local.ps1 -VerifyOnly`, then rerun `-StrictVerifyOnly`. Codex Desktop `26.707.3748.0` ships a `deep-research` descriptor that its runtime can intentionally remove from the local marketplace. The strict verifier allows only that exact version/name omission, only when the installed source plugin is complete and the local plugin directory is wholly absent; all other missing descriptors and all later package versions remain failures until separately validated.
...
- If verify-only reports disabled/expired/401/403, or manual refresh failed with `refresh_token_reused`, run the same script without `--verify-only` and finish the browser PKCE flow. The script writes only `$env:USERPROFILE\.codex\remote.json`, backs up the old file under `.codex\backups\remote-control-auth`, and must not write `.codex\auth.json` or `config.toml`. Proxy selection is automatic: environment proxy first, a listening `127.0.0.1:10808` second, otherwise direct; pass `--proxy ""` to force direct.
- Direct token exchange can fail with `unsupported_country_region_territory`; in that case use an explicitly configured working proxy. Do not force `http://127.0.0.1:10808` unless it is listening, and use `--proxy ""` only when intentionally forcing the direct path.
...
- For Codex Desktop 26.707, prefer `scripts\build-remote-control-native-replacement.ps1 -WorkRoot "<short-large-local-build-root>"` and let the helper detect the installed native version. Known mappings include `0.144.0-alpha.4` and `0.144.5`; historical `0.142.4` has a separate patch-apply-validated patch. Any other version needs exact matching parameters plus a validated `-PatchPathOverride`.
...
- If it reports `0.0.0`, or an older app-server version than the bundled native from the installed Desktop package, identify the matching Codex Rust source ref and workspace package version. The helper maps `rust-v0.144.0-alpha.4` and `rust-v0.144.5` to the current replacement patch and has a distinct historical patch for `rust-v0.142.4`; use `-PatchPathOverride` only after validating another version-specific patch.
