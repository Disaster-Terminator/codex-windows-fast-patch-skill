...
The helper checks the self-use fork `Disaster-Terminator/codex-windows-fast-patch-skill` on GitHub and synchronizes only the skill allowlist: `SKILL.md`, `agents`, `scripts`, `references`, and `assets`. Keep that fork synced from the `upstream` remote before publishing updates. 如果无法更新到最新版，则不要中断当前任务；继续使用本机已安装的当前版本完成工作，并在结果中说明未能更新。
...
- Use the Model Experience workflow for Fast Mode request/UI failures, API-key or hybrid hosts that lack service-tier requirement data, Ultra reasoning being hidden, new models hidden from the Desktop picker, or the compact Power slider falling back to the legacy picker. These symptoms share the same service-tier/model-picker area. Run `scripts\patch_codex_fast_mode_windows_msix.ps1 -OnlyModelExperience -DryRun` first; it checks the Fast request gate, Fast UI gate, custom-model metadata normalization, the Electron Power slider `harborEnabled` gate, and the independent Ultra picker gate, then repairs only the broken parts in one MSIX repack. `-OnlyCustomModels` remains a compatibility alias. Merge missing model metadata into `models_cache.json` only when read-only inspection proves the cache entry is absent; back up the cache first.
...
It patches Fast Mode in both the request path and the settings UI path. The request patch keeps the ChatGPT requirement lookup but treats non-ChatGPT hosts as locally eligible instead of returning false. The UI patch treats missing requirement data as unknown rather than denied, while still honoring an explicit `fast_mode = false`; this is required for API-key and hybrid hosts where the requirements query can return no data.
It also forces the configured custom model IDs through the Desktop model visibility filter and normalizes their model metadata at that boundary. A custom model that advertises `max` but not `ultra` is exposed as `ultra`, an empty `serviceTiers` list gains a local `priority` / Fast tier, and Ultra is added to the enabled effort set for the filtered result. The compact Work / Power picker opens the upstream Electron `harborEnabled` gate and removes its independent Ultra-slider gate. By default the forced model IDs are `gpt-5.6-sol`, `gpt-5.6-terra`, and `gpt-5.6-luna`; `models_cache.json` is not edited.
...
If the phone reports the Codex environment is expired after a native replacement, inspect the original installed native version before building. Prefer auto-detection from the installed native copy and use a short work root to avoid Windows path-length failures:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File "$env:USERPROFILE\.codex\skills\codex-windows-fast-patch\scripts\build-remote-control-native-replacement.ps1" -WorkRoot "G:\rc"
```

The build helper keeps the clone, Cargo cache, Rustup cache, temp directory, target directory, and any bootstrapped Windows SDK packages under `-WorkRoot`. When `-CodexSourceRef` and `-AppServerVersion` are omitted together, it copies the installed WindowsApps `app\resources\codex.exe` into `WorkRoot\tmp`, runs `--version` only on that copy, and selects a bundled version mapping; it never executes the WindowsApps binary in place. Codex Desktop `26.707.3748.0` maps to `rust-v0.144.0-alpha.4` / workspace `0.144.0-alpha.4`, while Desktop `26.707.12708.0` maps to `rust-v0.144.5` / workspace `0.144.5`; both use `references\remote-control-native-replacement.patch`. The `0.144.5` combination has completed MSVC build, MSIX dry-run/signed install, live marker, and isolated-auth endpoint verification, but enrollment and a phone-message round trip remain pending. For historical `rust-v0.142.4`, matching parameters select `references\remote-control-native-replacement-0.142.4.patch`; that patch has passed clean patch-apply validation only. Other source versions require matching explicit version parameters plus a validated `-PatchPathOverride`. Do not use GNU toolchain output for Windows MSIX replacement; use the MSVC target.

If MSVC is present but `kernel32.lib` is missing, the helper first searches for one coherent existing Windows SDK root/version containing matching x64 `kernel32.lib`, `ucrt.lib`, and headers; it does not mix independently discovered versions. Only when no usable SDK exists, it downloads `Microsoft.Windows.SDK.CPP` and `Microsoft.Windows.SDK.CPP.x64` version `10.0.26100.4188` into `<WorkRoot>\cache\windows-sdk-cpp`. Downloads use `.partial` files, validate the ZIP and expected payloads, and replace the cache only after validation; corrupt cached packages are deleted and downloaded again. Downloads honor existing `HTTPS_PROXY` / `HTTP_PROXY`; when neither is set, the helper uses `http://127.0.0.1:10808` only if that port is listening, otherwise it downloads directly. Keep `-WorkRoot` short and place it on the user's requested large local drive so the bootstrap stays off the system drive without triggering Cargo path-length failures.
...
- `-OnlyModelExperience`: inspect and selectively repair the Fast Mode request gate, missing-data Fast UI fallback, custom-model metadata normalization, and compact Power / Ultra picker gates together. Use this for API-key/hybrid Fast Mode, hidden Ultra, empty custom-model service tiers, hidden custom models, and the dependent compact Power slider. The legacy `-OnlyCustomModels` name is retained as an alias.
...
- `-CodexSourceRef <rust-vX> -AppServerVersion <X>`: always supply these as an exact matching pair when overriding auto-detection. `0.144.0-alpha.4` is full build/install/phone-message-round-trip validated with Codex Desktop `26.707.3748.0`; `0.144.5` is build/install/live-marker/isolated-auth validated with Desktop `26.707.12708.0` but still awaits enrollment and a phone-message round trip; the dedicated `0.142.4` patch is patch-apply validated only.
...
- The patch log includes `custom model metadata patch result` and `compact Power / Ultra patch result`; the patched model filter contains all configured custom model IDs plus the metadata and picker markers.
...
- The patched ASAR has `webview\assets\use-service-tier-settings-*.js` with the Fast Mode UI gate unblocked, `model-list-filter-*.js` with `CODEX_CUSTOM_MODEL_METADATA_V2_START`, and `model-and-reasoning-dropdown-*.js` with `CODEX_WORK_MODE_ACCESS_V1` plus `CODEX_ULTRA_SLIDER_V1`. The locale chunk has `enable_i18n` forced enabled, and browser_use feature chunks/main feature dispatch report in-app and external browser availability locally.
