...
- Use the Custom Models Only workflow when CLI or the provider model endpoint already exposes the requested model but Desktop hides it, or when Sol/Terra are filtered out and the compact Power slider falls back to the legacy picker. Run `scripts\patch_codex_fast_mode_windows_msix.ps1 -OnlyCustomModels -DryRun` first, then install from an external executor. Merge missing model metadata into `models_cache.json` only when read-only inspection proves the cache entry is absent; back up the cache first.
